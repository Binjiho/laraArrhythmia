...
function mGnb() {
    ...
    $('.js-gnb > li > a').off().on('click',function(e){
        if($(this).next('ul').length){
            ...
            $('.js-gnb > li > a').not(this).next('ul').stop().fadeOut(100);
            return false;
        }
    });
}
...
function mainVisual(){
    $('.js-main-visual').not('.slick-initialized').slick({
        ...
        customPaging: function (slider, i) {
			...
			return '<button>0'+ Number(i+1) + '</button>';
		},
        autoplay: true,
        autoplaySpeed: 3000,
        speed: 1000,
