...
@section('addScript')
    <script>
        ...
        /**
         * 상세보기 유무
         * 행사구분,년도,행사명,행사기간,행사장소,사전등록 기간,썸네일 이미지,초록접수 기간,홈페이지 URL
         */
        $(document).on('click', 'input[name="detail"]', function() {
            let detailData = $('input[name="detail"]:checked').val();

            if(detailData == 'Y'){
                $(".detailY").show();
                // $(".detailY").prop('disabled',false);
            }else{
                $(".detailY").hide();
            }
        });
        ...
        $(document).on('click', '#board_cancel', function(e) {
            ...
            if (confirm(msg)) {
                location.replace('{{ route('conference', ['code' => $code]) }}');
            }
        });
