                ...
                <div id="board" class="event-wrap board-wrap">
                    <div class="board-write">
                        <div class="write-form-wrap" id="popupPrepare">
                            <form id="board-frm" data-sid="{{ $_GET['surgery_sid'] ?? 0 }}" data-case="reviewer-regist" onsubmit="return false;">
                                <fieldset>
                                    <table class="inputTbl ">
                                        ...
                                        @forelse($list as $reviewer)
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="checkbox" name="reviewer_sid[]" id="reviewer_sid{{ $reviewer->user_sid }}" value="{{ $reviewer->user_sid }}" {{ in_array($reviewer->user_sid, ($reviewer_users ?? []) ) ? 'checked' : '' }} >
                                                </td>
                                            </tr>
                                        @empty
...
@section('addScript')
    <script>
        ...
        $('#board-frm').on('submit', function(e) {
            ...
            let reviewer_sid = $(":checkbox[name='reviewer_sid[]']:checked").map((i, el) => el.value).get();

            if (confirm('등록하시겠습니까?')) {

                let ajaxData = newFormData(boardForm);

                ajaxData.append("reviewer_sid", JSON.stringify(reviewer_sid));

                callMultiAjax('{{ route("surgery.data") }}', ajaxData);
            }
        });
    </script>
@endsection