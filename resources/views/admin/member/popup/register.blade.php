...
@section('contents')
    <div class="contents">
        <article class="sub-contents">
            <form action="{{ route('member.data') }}" method="post" id="register-frm" enctype="multipart/form-data" onsubmit="return false;" data-sid="{{ request()->sid }}" data-case="member-update" >
{{--                @csrf--}}
...
@section('addScript')
    ...
    <script>
        ...
        $(document).on('click', '#adminPassword', function () {
            if($(this).is(':checked')) {
        ...
        const getSid = (_this) => {
            return $(_this).closest('.absList').data('sid');
        }

        $(document).on('click', '.file_del', function() {
            actionConfirmAlert('삭제 하시겠습니까?', {
                'input': [
                    actionInput('#file_del', 'Y'),
                    actionInput('#fileName', ''),
                    actionInput(form + ' input[name=banner_image]', ''),
                ],
                'html': [
                    actionHtml('.attach', ''),
                ]
