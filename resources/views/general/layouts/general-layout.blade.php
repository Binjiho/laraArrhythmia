...
<head>
    @include('general.layouts.components.baseHead')
</head>
<body>
<div id="wrap" class="{{ $main_menu == 'main' ? "main" : "sub" }}">
    @include('general.layouts.include.header')
    <hr/>
    ...
    <section id="container" class="{{ $main_menu == 'main' ? "main" : "sub" }}">
        @if($main_menu !== 'main')
            @include('general.layouts.include.titArea')
        @endif
    ...
    @include('general.layouts.include.footer')

    <hr/>
</div>
<!-- //wrapper -->

@include('general.layouts.components.spinner')
@yield('addScript')
</body>
</html>
